...
struct nxp_adc_info {
	...
	int irq;
	struct iio_map *map;
//---------------------------------------------changed to 1 by rpdzkj ivy 2015.10.29  1ÐÐ
#if 1	//defined(CONFIG_ARM_NXP_CPUFREQ_BY_RESOURCE)
	struct iio_dev *iio;
...
//****************************************************added by rpdzkj for adc 2015.10.29
...
//****************************************************added by rpdzkj ivy for rp_adc 2015.10.29
int rp_adc_read(int channel)
{
	struct iio_chan_spec const *chan;
	int val = 0;
	int val2 = 0;
	...
	chan = &nxp_adc_iio_channels[channel];
	nxp_read_raw(adc_global->iio, chan, &val, &val2, 0);

	return val;
}
EXPORT_SYMBOL(rp_adc_read);
//****************************************************added by rpdzkj
...
static int __devinit nxp_adc_probe(struct platform_device *pdev)
{
	...
	*/
	...
	adc->iio = iio;
	//added rpdzkj ivy for rp_adc_read()  2015.10.29
	adc_global = adc;
