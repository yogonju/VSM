...
//static int cdc_encap_ioctl(struct inode *node, struct file *filp, unsigned int cmd, unsigned long arg)
static long cdc_encap_ioctl( struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err = 0;
	int ret = 0; // dxx 
	struct cdc_encap *encap = filp->private_data;
	...
	//case CDC_ENCAP_IOCTL_SUSPEND_ACK:
        ...
        case CDC_ENCAP_IOCTL_ERAD_DIALMODE:
            ret = copy_to_user((char *)arg, (char *)&(encap->ecm_ndis_dial_mode), sizeof(int));
            break;
	default:
		err = -EINVAL;
	}
...
static struct file_operations cdc_encap_fops = {
	...
	.unlocked_ioctl = cdc_encap_ioctl, // dxx
	.open =		cdc_encap_open,
