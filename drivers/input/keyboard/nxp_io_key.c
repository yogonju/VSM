...
#include <mach/platform.h>
#include <mach/soc.h>
//---------------------------------------------added by rpdzkj ivy 2015.10.29  ÐÐ
#include <mach/rpdzkj_debug.h>

//--------------------------------------------- by rpdzkj ivy 2015.11.26 end
#define GTP_GPIO_GET_VALUE(pin)         gpio_get_value(pin)
#define GTP_GPIO_SET_VALUE(pin,value)   gpio_set_value(pin,value)
#define GTP_GPIO_OUTPUT(pin,level)      gpio_direction_output(pin,level)
#define GTP_GPIO_REQUEST(pin, label)    gpio_request(pin, label)
#define GTP_GPIO_FREE(pin)              gpio_free(pin)

int press_flags_one = 0;
int press_flags_two = 0;
...
static void nxp_key_event_wq(struct work_struct *work)
{
	...
	//unsigned int keyio = code->io;
	int press = 0;
	u_long flags;
	//TK
	GTP_GPIO_REQUEST(CFG_IO_BUTTON_UP, "KEYPAD_UP_IRQ");
	GTP_GPIO_REQUEST(CFG_IO_BUTTON_DO, "CFG_IO_BUTTON_DO");
	GTP_GPIO_REQUEST(CFG_IO_BUTTON_LG, "CFG_IO_BUTTON_LG");
	...
	if(press != code->keystat) {
		...
		if (KEY_STAT_PRESS == press) {
			...
			if( keycode== KEY_VOLUMEDOWN || keycode== KEY_VOLUMEUP ||keycode== KEY_POWER ){	
			...
			if( keycode== KEY_UP ){ 
				//printk("else keycode==KEY_UP \n");
				GTP_GPIO_SET_VALUE(CFG_IO_BUTTON_UP,0);				
				//mdelay(1);
				if (!GTP_GPIO_GET_VALUE(CFG_IO_BUTTON_DO))
					{	
					input_report_key(key->input, keycode, 1);
					input_sync(key->input);
					input_report_key(key->input, keycode, 0);
					...
					//printk("CFG_IO_BUTTON_DO = 1\n");
					input_report_key(key->input,KEY_RIGHT, 1);
					input_sync(key->input);	
					input_report_key(key->input,KEY_RIGHT, 0);
					input_sync(key->input);		
				}
				gpio_direction_input(CFG_IO_BUTTON_UP);
			}
			if( keycode== KEY_DOWN ){
				//printk("else keycode==KEY_DOWN \n");
				GTP_GPIO_SET_VALUE(CFG_IO_BUTTON_UP,0);	
				//mdelay(1);
				if (!GTP_GPIO_GET_VALUE(CFG_IO_BUTTON_LG))
					{	
					input_report_key(key->input, keycode, 1);
					input_sync(key->input);
					input_report_key(key->input, keycode, 0);
					...
					input_report_key(key->input,KEY_LEFT, 1);
					input_sync(key->input);	
					input_report_key(key->input,KEY_LEFT, 0);
					input_sync(key->input);	
				}
				gpio_direction_input(CFG_IO_BUTTON_UP);
			}
	...
	pr_debug("key io:%d, code:%4d %s\n", code->io, keycode,
		...
		if(press == code->keystat ){
			code->keystat = press;
		if (KEY_STAT_PRESS != press) {
			if(press_flags_one == 0) {
			if( keycode== KEY_UP ){ 
				//printk("keycode==KEY_UP \n");
				GTP_GPIO_SET_VALUE(CFG_IO_BUTTON_UP,0);				
				//mdelay(1);
				if (!GTP_GPIO_GET_VALUE(CFG_IO_BUTTON_DO))
					{	
			...
			if(press_flags_two == 0) {
			if( keycode== KEY_DOWN ){
				//printk("keycode==KEY_DOWN \n");
				GTP_GPIO_SET_VALUE(CFG_IO_BUTTON_UP,0);	
				//mdelay(1);
				if (!GTP_GPIO_GET_VALUE(CFG_IO_BUTTON_LG))
					{	
	...
	GTP_GPIO_FREE(CFG_IO_BUTTON_UP);
	GTP_GPIO_FREE(CFG_IO_BUTTON_LG);
	GTP_GPIO_FREE(CFG_IO_BUTTON_DO);
...
static int nxp_key_probe(struct platform_device *pdev)
{
	...
	input->keycodemax = plat->bt_count * 2;	// f or long key
	...
	for (i=0; keys > i; i++, code++) {
		...
		if(code->keycode== KEY_POWER || code->keycode== KEY_VOLUMEUP || code->keycode== KEY_VOLUMEDOWN){
		//TK make button INT 
		GTP_GPIO_REQUEST(code->io, "KEYPAD_INT_IRQ");
		GTP_GPIO_SET_VALUE(code->io,1);
		GTP_GPIO_FREE(code->io);
		//END
		}
		//---------------------------------------------added by rpdzkj ivy 2015.10.29  1ÐÐ
		//enable_irq(gpio_to_irq(code->io));
	} 
	platform_set_drvdata(pdev, key);

	printk("------ %s  %s-%d:succedd ------\n", __FILE__, __func__, __LINE__);
