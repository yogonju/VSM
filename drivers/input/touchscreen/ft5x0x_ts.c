...
#include <mach/platform.h>
#include <mach/soc.h>
#ifdef  CONFIG_NXP_DISPLAY_MIPI
	#include <linux/gpio.h>
	#include <asm/gpio.h>
#endif /* CONFIG_NXP_DISPLAY_MIPI*/
...
static void ft5x0x_ts_report(void)
{
	...
	for (i = 0; event->touch_points > i; i++) {
		if (1 != event->st[i]) {
			...
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X, event->y[i]);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, event->x[i]);
			input_mt_sync(ts->input_dev);
	...
	if (event->touch_points == 1) {
		...
		input_report_abs(ts->input_dev, ABS_X, event->y[i]);
		input_report_abs(ts->input_dev, ABS_Y, event->x[i]);
		input_report_abs(ts->input_dev, ABS_PRESSURE, event->press[i]);
		//printk("single touch : (x=%4d, y=%4d)\n", event->y[i], event->x[i]);
	}
...
//		err =ft5x0x_i2c_write( NULL, 0);
//		err = ft5x0x_i2c_set_reg(0x80,0x30);
		err = ft5x0x_i2c_read(buf,1);
		if (err > 0)
			break;
	}
	if(err <= 0) {
...
//	err = request_irq(client->irq, ft5x0x_ts_interrupt,IRQF_TRIGGER_FALLING, "ft5x0x_ts", ts);
	err = request_irq(client->irq, ft5x0x_ts_interrupt, IRQF_DISABLED | IRQF_TRIGGER_FALLING, "ft5x0x_ts", ts);
	if (err < 0) {
